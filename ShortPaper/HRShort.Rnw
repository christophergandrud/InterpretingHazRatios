...
We can use a similar procedure for splines. Penalized splines \citep{Eilers1996}, a commonly used way of showing more complex nonlinear effects than polynomials allow \cite[see][]{Keele2008}. They are basically ``linear combinations of B-spline basis functions'' \citep[5]{Strasak2009} joined at points in the range of observed values of $x$ called ``knots'' \citep[50]{Keele2008}. A Cox PH model with one penalized spline is given by:
%
\begin{equation}
...
In this paper I have reviewed a number of important insights about avoiding proportional hazard model misspecification. Looking out for and properly estimating many of the interactions and nonlinearities that can cause biased estimates is crucial for political scientists. They can actually be factors that we are interested in studying. So we need tools to fully explore them, including their associated uncertainty. Exploring results estimated from Cox-type models and their associated uncertainty is often difficult with currently available software. So, the main contribution of this paper has been to create and illustrate a new R package--\emph{simPH}--that makes it considerably easier to effectively explore and present with visual weighting a variety of quantities of interest. It allows users to explore both central and shortest probability intervals, the latter of which can be particularly useful for results estimated from Cox-type models. \emph{simPH} is freely available for researchers to download using instructions in the Appendix.
