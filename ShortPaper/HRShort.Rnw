...
Again, we can use a similar procedure for splines. Penalized splines \citep{Eilers1996}, for example, are basically ``linear combinations of B-spline basis functions'' \citep[5]{Strasak2009} joined at points in the range of observed values of $x$ called ``knots'' \citep[50]{Keele2008}. A Cox PH model with one penalized spline is given by:
%
\begin{equation}
...
\end{equation}
%
We can use King et al.'s simulation procedure for many other quantities of interest including hazard rates and even marginal effects for interactions \cite[see][]{Brambor2006}. 
