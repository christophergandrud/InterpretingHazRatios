...
<<eval=FALSE>>=
devtools::install_github("simPH", [REMOVED FOR BLIND REVIEW])
@

\noindent You need to have the \emph{devtools} package \citep{R-devtools} set up to install \emph{simPH}.

For more information about \emph{simPH} see: [REMOVED FOR BLIND REVIEW]. Please report any bugs to [REMOVED FOR BLIND REVIEW].
