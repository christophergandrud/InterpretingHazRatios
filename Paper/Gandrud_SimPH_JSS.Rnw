...
Now simply simulated the hazard ratios for a sequence of values with \code{coxsimSpline} and graph the results with \code{simGG}.
...
There are a few syntax points to note. First we tell \code{coxsimSpline} the covariate to estimate for using the \code{bspline} argument and the same syntax we used to enter the covariate as a penalized spline, i.e. \code{pspline(stafcder, df = 4)}. Second, we specified the vector containing the observed \code{stafcder} data with the \code{bdata} argument. This is important for \code{coxsimSpline} to be able to accurately find the spline knots. Finally, the \code{palpha} argument was added to \code{simGG} to change the transparency of the dots in order to highlight the range of the distribution more effectively. 
...
Again we can use \pkg{ggplot2} to change the plot that \code{simGG} creates. Imagine that we placed the outcome of \code{coxsimSpline} with the argument \code{spine = TRUE} into an object called \code{SimPlot}. We can then use \pkg{ggplot2} to change the y-axis range so that it is comparable to Figure \ref{Spline1}'s:
