...
The \cite{cox1972} Proportional Hazards (PH) model is used in a wide range of disciplines, including epidemiology and political science, to study time to event data. However, many \add[CG]{researchers} poorly communicate quantities of interest and associated uncertainty estimated with these models. This is especially true when estimates are generated from interactions and nonlinearly transformed continuous variables. These transformations can be especially useful with Cox PH for correctly specifying models that would otherwise have nonproportional hazards, a key violation of the model's assumptions. These effects are difficult to interpret using coefficient tables and available computational tools. In addition, estimation uncertainty around quantities of interest from Cox PH models is hard to assess because these quantities are on asymmetric and nonlinear scales. 

This article aims to improve the use of Cox PH models. It first briefly discusses what a Cox PH model is and previous research on how time interactions and nonlinear effects can be helpful for correctly specifying models that would otherwise violate the proportional hazards assumption (PHA). Second, it advocates using shortest probability intervals and visual-weighting to display simulated quantities of interest from interactive, nonlinear, and even \change[CG]{`vanilla'}{standard} linear estimates from Cox PH models.  \change[CG]{The latter type of interval is}{Shortest probability intervals are} often more appropriate for Cox PH model results. Third, it demonstrates how the new \proglang{R} \citep{CiteR} package \pkg{simPH} \citep{R-simPH} makes it easy to implement these techniques. The package is freely available on the Comprehensive \proglang{R} Archive Network. \change[CG]{Quantities of interest}{} \pkg{simPH} can simulate and plot \change[CG]{include}{quantities of interest, including} hazard ratios, first differences, relative hazards, marginal effects, and hazard rates from linear, linear multiplicative interactions between covariates, time interactions, and nonlinear transformations of continuous variables. Hypothetical and empirical examples are used to illustrate \pkg{simPH}'s syntax and capabilities. 
...
\end{equation}
%
where $h_{0}(t)$ is the baseline hazard, i.e., the instantaneous rate of \change[CG]{a transition}{the event of interest occuring} at time $t$ when all of the covariates are zero. $\mathbf{\beta}$ is a vector of coefficients and $\mathbf{X}_{i}$ is the vector of covariates for unit $i$.
...
Because of their role in correcting for violations of the proportional hazards assumption, it is especially important to have accessible tools for exploring estimates and associated uncertainty from time-interactions and nonlinear transformations of continuous variables. 

\section[Show estimates]{Show estimation uncertainty}

How can we effectively examine and communicate both the point estimates of and our uncertainty about quantities of interest from time-interactive and nonlinear effects? \change[CG]{In this section I advocate showing these results using}{I advocate illustrating these measures using} simulations, shortest probability intervals, and visually-weighted plots. These tools are useful for showing \change{`vanilla'}{standard} linear effects estimated from Cox PH models as well.

\subsection{Post estimation simulations}

Following \cite{King2000}, \cite{Licht2011} proposes post-estimation simulation techniques to make it easier to estimate the uncertainty surrounding quantities of interest for time interactions like first differences and relative hazards. See \citet[352-353]{King2000} for a discussion of alternative approaches including fully Bayesian Markov-Chain Monte Carlo techniques and \change[CG]{bootstraping}{bootstrapping}. The main difference between these three approaches is how the parameters are drawn. Using the post-estimation simulation technique, we first find the parameter point estimates for $\hat{\beta_{1}}$ and $\hat{\beta_{2}}$. Second, we draw $n$ values of $\beta_{1}$ and $\beta_{2}$ from multivariate normal distributions with means $\beta_{1}$ and $\beta_{2}$ and \change[CG]{variance}{variances} specified by the parameters' estimated covariance. Third, we use these simulated values to calculate a quantity of interest, such as the first difference or relative hazard, for a range of times as well as specified values of $x_{j}$ and $x_{l}$ (as appropriate). Finally, we plot the results. The simulation technique allows us to estimate full time-interactive effects, how they change over time, substantively evaluate the effects, and show uncertainty surrounding the estimates.
...
We can use a similar procedure for splines. Penalized splines \citep{Eilers1996} are a commonly used way of showing more complex nonlinear effects than polynomials \cite[see][]{Keele2008}. \change[CG]{They involve ``linear combinations of B-spline basis functions'' joined at points in the range of observed values of $x$ called ``knots''.}{A B-spline basis for penalized spline estimation provides a means for linear combinations of spline basis functions on the complete domain of the basis, not only at the knots.}
...
Age is clearly a time-varying quantity. Not only does it change over time in the way a person's income (probably) does, for example, but in effect it has a linear interaction with time. However, in our model we do not explicitly interact age with time as we will interact other variables with time in later examples. Because of this we can treat the variable the same way as personal income for the purposes of creating simulations with \pkg{simPH} and so can use the \code{coxsimLinear} function:
...
We can see that each piece of prospective legislation is identified with a case number in the \code{caseno} variable. The \code{event} variable records if the event of interest occurred (i.e. the legislation was passed) or not. Observation intervals \code{begin} at 0 and extend to the \code{end} when either the event occurs, \emph{or} one of the covariate values change. 

If we create a time interaction by using the \code{end} variable as the time value, we will inaccurately measure the interaction for failures before this time. \change[CG]{Imagine we build a simple interaction between a binary variable and time. One unit has the value one for the binary variable and this unit experiences an event of interest at time 1,000 in a data set formatted as above. The time interaction would then be treated as 1,000 for all failures at time  This would be inaccurate. At time 500, for example the interaction should be 500, not 1,000.}{Imagine we build an interaction between} \code{end} \add[CG]{time and a dichotomous variable with the value 1 for the second unit in our data set. The interaction term would be 1,246. This is accurate at the second unit's event time $t_{m}$. However, it is inaccurate at all times $t_{n}$, where  $t_{n} < t_{m}$. When another unit experiences an event at a time $t_{n}$ before $t_{m}$ we should use the second unit's time interaction value at $t_{n}$ to find the partial likelihood, rather than the value at $t_{m}$.}

To solve this problem, \pkg{simPH} includes the \code{SurvExpand} function. It expands a data set out into equally spaced intervals. It keeps all intervals that end at an observed event time and when a covariate changes. These are the only time points that are \change[CG]{interesting to the Cox PH model}{needed to find the partial likelihood.} \change[CG]{and r}{R}emoving unneeded periods helps save memory. The resulting data frame allows us to create accurate time interactions. Here is how we use the function with the \code{GolubEUPData} data:
