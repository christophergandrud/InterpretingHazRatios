...
Once we have created the \code{Sim1} object containing the simulations, we can simply use the \code{simGG} method to plot the results. The \code{xlab} and \code{title} arguments allow us to customize the x-axis label and plot title, respectively. The plots use ribbons to show the simulation distributions. We adujusted the median line size with the \code{lsize = 0.5} argument. This makes the particular plot clearer. Finally, the \code{legend = FALSE} argument hides the legend describing the fitted value comparisons that are being made between $Xj$ and $Xl$ as we are only comparing the values 1 and 0.\footnote{The default is for the legend to be shown. This argument follows the \pkg{ggplot2} syntax for creating plot guides, so to show the legend: \code{legend = "legend"}.} 
