...
\begin{enumerate}
	...
	\item Simulate parameter estimates and calculate quantities of interest--e.g., relative hazards, first differences, hazard ratios, marginal effects for linear interactions, or hazard rates--using the \pkg{simPH} command corresponding to the variable type. See Table~\ref{simTable} for a summary of the simulation commands.
...
To create this figure we first estimate the survival model using the \code{coxph} command from the \pkg{survival} package. Load the package and data \code{GolubEUPData} that is included with \pkg{simPH}.
...
Then create the log-time interactions. Notice the use of the command \code{tvc} to create the log-time interactions. The command is included with the \pkg{simPH} package. The \code{data} argument let's us specify the data frame where our covariate and time variables are. The covariate we wish to interact is specified with \code{b} and the time variable with \code{tvar}. Finally, we specify the function of time with the \code{tfun} argument.
