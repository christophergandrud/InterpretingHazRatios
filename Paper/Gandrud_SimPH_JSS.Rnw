...
To illustrate \pkg{simPH}'s syntax and capabilities, let's use it to replicate key figures and findings from \cite{Licht2011} and \cite{Keele2010}.
...
Notice the use of the command \code{tvc} to create the log-time interactions. It is included with the \pkg{simPH} package.

Now we can take the \code{M1} model object created by \code{survival} and simulate the first difference for qualified majority voting from days 80 through 2000 after deliberation has begun.

<<TVCModel2, eval=FALSE, tidy=FALSE>>=
...
Figure \ref{TermPlot} uses \code{termplot} to plot the log hazard of the effect and the standard errors. Apart from showing related but different quantities of interest, Figure \ref{TermPlot} gives us less useful information than Figure \ref{Spline1} about the probability distribution that the parameter is from. By only describing the distribution of the parameter with the mean values and standard errors, Figure \ref{TermPlot} makes it difficult for us to know what the distribution is. Figure \ref{Spline1} simulates this distribution and shows it directly.
