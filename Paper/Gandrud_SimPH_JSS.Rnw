...
%
\begin{equation}
    \%\triangle h_{i}(t) = (\mathrm{e}^{g(x_{j} - x_{l})} - 1) * 100.
\end{equation}
%
...
We can use this post-estimation simulation technique for virtually any quantity of interest estimated from Cox PH models including marginal effects for multiplicative interactions \cite[see][]{Brambor2006} and plain linear effects as well. 
...
\noindent We told \code{coxsimLinear} that we wanted to simulate relative hazards (the default quantity of interest) based on our model object \code{M1}. We specified which variable to find hazard ratios for using the \code{b} argument. The \code{Xj} argument lets us set the fitted values of $x_{j}$; in this case a sequence of values between -15 and 19 at intervals of 1. The smaller the interval, the smoother the resulting graph will look. To graphically present the results now in the \code{Sim1} object we can use the \code{simGG} method. The following code creates the left panel of Figure~\ref{LinearPlot}:
...
Notice that the time interaction varibles are added to the original \code{GolubEUPData} using the original variable names plus a suffix denoting the log transformation.
