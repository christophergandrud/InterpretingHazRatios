...
If we create a time interaction by using the \code{end} variable as the time value, we will inaccurately measure the interaction for failures before this time. \change[CG]{Imagine we build a simple interaction between a binary variable and time. One unit has the value one for the binary variable and this unit experiences an event of interest at time 1,000 in a data set formatted as above. The time interaction would then be treated as 1,000 for all failures at time  This would be inaccurate. At time 500, for example the interaction should be 500, not 1,000.}{Imagine we build an interaction between} \code{end} \add[CG]{time and a dichotomous variable with the value 1 for the second unit in our data set. The interaction term would be 1,246. This is accurate at the second unit's event time $t_{j}$. However, it is inaccurate at all times $t_{i}$, where  $t_{i} < t_{j}$. When another unit experiences an event at a time $t_{i}$ before $t_{j}$ we should use the second unit's time interaction value at $t_{i}$ to find the partial likelihood, rather than the value at $t_{j}$.}
