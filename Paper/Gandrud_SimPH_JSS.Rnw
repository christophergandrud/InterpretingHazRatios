...
\emph{simPH} is currently available for download from CRAN\footnote{\url{http://cran.r-project.org/web/packages/simPH/}} and the latest development version is available on GitHub.\footnote{\url{https://github.com/christophergandrud/simPH}} To install it in R type the following code into your R Console:
...
\subsubsection*{Figure \ref{Spline2}}
