...
This post-estimation technique is not limited to time-varying covariates, but can be used for virtually any quantity of interest estimated from Cox-type models including and marginal effects for interactions \cite[see][]{Brambor2006} and plain linear effects. 
...
\begin{figure}
  ...
  \label{Spline2}
<<Spline2Fig2, echo=FALSE, tidy=FALSE, message=FALSE, out.width="9cm", out.height="8cm", cache=TRUE, dev='pdf'>>=
