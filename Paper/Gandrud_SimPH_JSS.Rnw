...
options(prompt = "R> ", continue = "+  ", width = 70, useFancyQuotes = FALSE, comment=NA)
...
\end{equation}
%
where $h_{0}(t)$ is the baseline hazard, i.e., the instantaneous rate of the event of interest occuring at time $t$ when all of the covariates are zero. $\mathbf{\beta}$ is a vector of coefficients and $\mathbf{X}_{i}$ is the vector of covariates for unit $i$.
...
%
\begin{equation}
	\%\triangle h_{i}(t) = (\mathrm{e}^{(x_{j} - x_{l})(\beta_{1} + \beta_{2}f(t))} - 1) \cdot 100.
\end{equation}
%
...
We can use a similar procedure for splines. Penalized splines \citep{Eilers1996} are a commonly used way of showing more complex nonlinear effects than polynomials \cite[see][]{Keele2008}. A B-spline basis for penalized spline estimation provides a means for linear combinations of spline basis functions on the complete domain of the basis, not only at the knots.
...
If we create a time interaction by using the \code{end} variable as the time value, we will inaccurately measure the interaction for failures before this time. Imagine we build an interaction between \code{end} time and a dichotomous variable with the value 1 for the second unit in our data set. The interaction term would be 1,246. This is accurate at the second unit's event time $t_{m}$. However, it is inaccurate at all times $t_{n}$, where  $t_{n} < t_{m}$. When another unit experiences an event at a time $t_{n}$ before $t_{m}$ we should use the second unit's time interaction value at $t_{n}$ to find the partial likelihood, rather than the value at $t_{m}$.
