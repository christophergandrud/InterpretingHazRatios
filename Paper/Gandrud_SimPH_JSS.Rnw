...
The left panel of Figure~\ref{TVCQMV} recreates Licht's \citeyearpar{Licht2011} figure showing the first difference of a log-time interaction for how the effect of the QMV procedure on legislative deliberation time changes as the number of days of deliberation increases \citep[see][236]{Licht2011}.\footnote{Her original figure was not in terms of a percentage difference to make it more comparable to a figure in Golub and Steunenberg's original. I present the results in terms of percentage difference, as the first difference is commonly reported. I have not separated out the pre and post Single European Act time periods for simplicity. There are discrepancies in the estimates presented in her figures and those here. These are caused by differences in how the underlying model was estimated in \proglang{Stata} compared to \proglang{R}. Finally, I also examined whether or not nonlinearity functional forms would be better fits than time interactions as per our discussion above. However, I found no evidence of this.} To create this figure we first load the data \code{GolubEUPData}. This data set is included with \pkg{simPH}.

<<TVCModel1.1, tidy=FALSE, echo=TRUE, message=FALSE, warning=FALSE>>=
data("GolubEUPData")
@

Then we create the log-time interactions. Before we do this, let's look at the data format:

<<TVCModel1.1.1, tidy=FALSE, echo=TRUE, message=FALSE, warning=FALSE>>=
head(GolubEUPData[, 2:5])
@

We can see that each piece of prospective legislation is identified with a case number in the \code{caseno} variable. Observation interval's \code{begin} at 0 and extend to the \code{end} when either an even happens, or one of the covariates changes values. If we simply create a time interaction by using the \code{end} time as the time value, then we will generally over measure the interaction when there are failures at times before this. For example, imagine we create a simple interaction between a dummy variable and time. For one unit that experiences an event of interest at time 1,000 in a data set formatted as above, the time interaction would be treated as 1,000 for all failures at time $t \leq 1000$. This would be inaccurate as at time 500, for example the interaction should be 500, not 1,000.

To solve this problem, \pkg{simPH} includes a function called \code{SurvExpand} to expand a data set out into equally spaced intervals. This allows for the creation of accurate time interactions. This is how we would use the function with the \code{GolubEUPData} data:
