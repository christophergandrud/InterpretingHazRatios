...
To solve this problem, \pkg{simPH} includes a function called \code{SurvExpand} to expand a data set out into equally spaced intervals. It keeps all intervals that end at an observed event time or when a covariate changes. This allows us to create accurate time interactions. Here is how we use the function with the \code{GolubEUPData} data:
...
The paper can be completely reproduced from source code files available at: \url{https://github.com/christophergandrud/InterpretingHazRatios}.
