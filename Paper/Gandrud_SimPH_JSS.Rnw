...
This post-estimation technique is not limited to time-varying covariates, but can be used for virtually any quantity of interest estimated from Cox-type models including marginal effects for interactions \cite[see][]{Brambor2006} and plain linear effects. 
...
{\scriptsize {The figures' points show middle 95 percent of the simulations at each value of FDA staff using either the central or shortest probability interval. \\ As in Licht's original the time period plotted is truncated from 80 to 2000 to make the estimates more easily interpretable.}}
\end{figure}
