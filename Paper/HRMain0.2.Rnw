...
Though current capabilities for showing quantities of interest for splines are marginally improved by the simulation capabilities of \emph{simPH}, there are currently no easy to use ways to visually explore time-varying effects. I will recreate plots from \cite{Licht2011} demonstrate how this can be done with \emph{simPH}.\footnote{As I mentioned earlier, she makes the Stata source code available. \emph{simPH} largely makes it much easier to implement these methods.} 

She replicated analyses from \cite{Golub2007} on the duration of deliberation about E.U. directives.
