...
\begin{enumerate}
	...
	\item If non-proportionality still exists after including time interactions, consider another model that does not use the PHA. Continue to test for time-period specific biases.
