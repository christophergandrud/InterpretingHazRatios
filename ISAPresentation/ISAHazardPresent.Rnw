...
%%%%%%%%%%%%%%% Motivation %%%%%%%%%%%%%%
\section{Motivation: Misspecification \& Misinterpretation}
\frame{
...
\frame{
	...
	\begin{itemize}
		...
		\item<2-> Keele (2010): non-linear effects,
...
\frame{
	...
	{\LARGE{Another cause:}} {\large{\textbf{non-linear effects} \\
	\vspace{1cm}
	Keele suggests testing for non-linearity and including \textbf{polynomials/splines} \textbf{before testing} the PHA. \\
	\vspace{0.5cm}
...
\frame{
	...
	\vspace{0.5cm}
	For example, the \textbf{number of FDA Drug Review staff} has a non-linear effect.
	}}
}
...
\frame{
	...
	\begin{itemize}
		...
		\item Options for finding highest density regions (Hyndman 1996), especially useful for quanties on non-linear scales.
